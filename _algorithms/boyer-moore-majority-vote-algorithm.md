...
algorithm:
  ...
  space_complexity: $\Theta(1)$[^time-complexity]
  aliases: []
  level: yellow
description: >
...
より正確には $i = 0, 1, \dots, N - 1$ の順に、$a _ j$ が取り除かれれていないかつ $a _ j \neq a_ i, j \lt i$ となるような $j$ が存在するならば $a _ j$ と $a _ i$ を取り除く、という操作を繰り返す。
...
これによって得られた出力の候補が実際に過半数を占めるかどうかは、もう一度 $a$ を走査することで確認することが出来る。
...
[^space-complexity]: $\lvert S \rvert$ を保持するためには $\Theta(\log(N))$ bits が必要になる。本ページでは Word RAM を仮定し、これを $\Theta(1)$ として扱っている。
